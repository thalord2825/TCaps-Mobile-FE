...
export function BatchCard({ item, onViewDetails, onApproveTransfer }: BatchCardProps) {
  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        <View style={{ backgroundColor: colors.accent, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999 }}>
          <Text style={{ color: isDark ? "#0b1020" : "#FFFFFF", fontWeight: "800" }}>{item.productCode}</Text>
        </View>
        <View
          style={{
      ...
      {/* Info rows */}
      <View style={{ gap: 6 }}>
        <Text style={{ color: colors.textHigh, fontWeight: "700" }}>{item.factory}</Text>
        <Text style={{ color: colors.textLow }}>{item.stage}</Text>
        <Text style={{ color: isDark ? colors.textMuted : colors.textLow, fontSize: 12 }}>
      ...
      <View style={{ flexDirection: "row", gap: 10 }}>
        ...
        >
          <Text style={{ color: isDark ? colors.textMedium : colors.textHigh, fontWeight: "700" }}>Xem chi tiết</Text>
        </Pressable>
        <Pressable
          accessibilityRole="button"
        ...
        >
          <Text style={{ color: isDark ? colors.accent : "#FFFFFF", fontWeight: "800" }}>Duyệt / Chuyển</Text>
        </Pressable>
      </View>
    </View>
  );
